...
def init_public_storage(
    ...
    try:
        ...
        try:
            ...
            logger.warning("Public object storage bucket not reachable (%s): %s", _bucket, exc)
        logger.info("Public object storage storage connected: %s", _bucket)
    except Exception as exc:
        logger.warning("Public object storage not available: %s. Public uploads disabled.", exc)
        _client = None


def upload_public_bytes(path: str, data: bytes, content_type: str = "application/octet-stream") -> str:
    if _client is None:
        raise RuntimeError("Public object storage not initialized")
...
def delete_public_object_from_url(url: str | None) -> None:
    ...
    try:
        ...
        logger.warning("Public object storage delete failed for %s: %s", key, exc)
