        ...
        for attempt in range(1, MAX_RETRIES + 1):
            try:
                ...
                if not _is_retryable(e):
                    logger.warning("object storage %s non-retryable error: %s", func.__name__, e)
                    raise
                if attempt < MAX_RETRIES:
                    delay = RETRY_DELAY * attempt
                    logger.warning(
                        "object storage %s failed (attempt %d/%d): %s. Retrying in %.1fs...",
                        func.__name__,
                        attempt,
                        MAX_RETRIES,
                        e,
                    ...
                    time.sleep(delay)
        logger.error("object storage %s failed after %d attempts: %s", func.__name__, MAX_RETRIES, last_exc)
        raise last_exc
...
def init_storage(
    ...
    try:
        ...
        if sign_host and sign_host != urlparse(endpoint_url).netloc:
            ...
            logger.info("object storage storage connected: %s/%s (presign via %s)", endpoint, bucket, sign_host)
        else:
            _signing_client = _client
            logger.info("object storage storage connected: %s/%s", endpoint, bucket)

        _ensure_bucket()
    except Exception as e:
        logger.warning("object storage storage not available: %s. File uploads disabled.", e)
        _client = None
...
def _ensure_bucket():
    ...
    try:
        ...
        logger.warning("object storage bucket not reachable (%s): %s", _bucket, e)
