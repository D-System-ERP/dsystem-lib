...
async def connect(rabbitmq_url: str):
    ...
    try:
        ...
        await _channel.declare_exchange("dsystem.events", aio_pika.ExchangeType.TOPIC, durable=True)
        await _channel.declare_exchange("dsystem.rpc", aio_pika.ExchangeType.DIRECT, durable=True)
...
async def publish(routing_key: str, body: dict, exchange: str = "dsystem.events"):
    if not _channel:
